    ...
    export default{
        ...
        methods:{
            async registerUser(e){
                ...
                try{
                    const { data, pending, error, refresh } = await useFetch('http://localhost:1337/api/auth/local/register', requestOptions);

                    if(data._value){
                        const userStore = useUserStore();
 
                        userStore.setUser(data.value.jwt, data.value.user)

                        navigateTo({ path:'/' })
                    }else{
                        alert("Errore nella registrazione")
                        console.log("err",error.value.data.error.details.errors)

                        const err = error.value.data.error;
